...
describe("Gas1", function () {
    ...
    it("Checks a transfer", async function () {
        ...
        const transferTx = await gasContract.transfer(
            addr1.address,
            100,
            "acc1"
        );
    ...
    it("Checks an update", async function () {
        ...
        const transferTx1 = await gasContract.transfer(
            addr1.address,
            300,
            "acc1"
        );
        await transferTx1.wait();
        const transferTx2 = await gasContract.transfer(
            addr1.address,
            200,
            "acc1"
        );
        await transferTx2.wait();
        const transferTx3 = await gasContract.transfer(
            addr1.address,
            100,
            "acc1"
        );
        await transferTx3.wait();
        const transferTx4 = await gasContract.transfer(
            addr2.address,
            300,
            "acc2"
        );
        await transferTx4.wait();
        const transferTx5 = await gasContract.transfer(
            addr2.address,
            100,
            "acc2"
        );
        ...
        const updateTx = await gasContract.updatePayment(
            owner.address,
            1,
            302,
            3
